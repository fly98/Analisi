...
async function estraiDaMessaggi(env, grezzi, esistente) {
  ...
  const sistema = [
    ...
    "   riguardano il soggiorno in concreto: come intende pagare (per esempio la tassa di soggiorno in",
    "   contanti), deposito bagagli, orari, culla, animali, allergie, parcheggio, fattura, arrivi tardivi.",
    ...
    "",
    "   COME SCRIVERE LA NOTA: deve dire la COSA, non che la cosa e' stata nominata.",
    "   Chi legge non ha i messaggi sotto gli occhi e deve capire tutto dalla sola nota.",
    "   Scrivi frasi brevi e concrete, con i dettagli utili (quantita', giorni, orari, nomi).",
    "   NO: 'Domanda su parcheggio'      SI: 'Chiede se e' facile parcheggiare in zona'",
    "   NO: 'Richiesta deposito bagagli'  SI: 'Vuole lasciare i bagagli dopo il check-out, riparte alle 20'",
    ...
    "",
    "   DUE REGOLE SEVERE. Sono precauzioni, non la correzione di un errore accaduto:",
    "   le note scritte a mano da Filippo non vengono mai toccate, quelle le scrive solo lui.",
    ...
    "      Un ospite che chiede la fattura ha chiesto la fattura, punto: non aggiungere che cosa",
    "      bisognerebbe fare in cassa o in amministrazione, non e' compito tuo saperlo.",
    "      NO: 'Ricevuta bloccata'   SI: 'Chiede fattura via email al check-out, P.IVA 03169750597'",
...
// Booking "messaggi degli ospiti in attesa" (business): Filippo vuole continuare a riceverle
// (sono un promemoria utile), ma sparire da sole dopo 7 giorni invece di restare per sempre.
async function runCleanupBookingMessaggiAttesa(env) {
  const account = "business";
  const tok = await getGmailAccessTokenFor(env, account);
  if (!tok || !tok.access_token) {
    console.log("Cleanup Booking messaggi in attesa: auth fallita", tok);
    return;
  }
  const q = "from:booking.com subject:(attesa) older_than:7d";
  let total = 0;
  for (let page = 0; page < 5; page++) {
    const listResp = await fetch(
      "https://gmail.googleapis.com/gmail/v1/users/me/messages?" + new URLSearchParams({ q, maxResults: "500" }),
...
// Avvisi sicurezza Google sul business: restano visibili 7 giorni poi si cestinano da soli.
async function runCleanupGoogleSecurityBusiness(env) {
  const account = "business";
  const tok = await getGmailAccessTokenFor(env, account);
  if (!tok || !tok.access_token) {
    console.log("Cleanup Google security business: auth fallita", tok);
    return;
  }
  const q = "from:accounts.google.com subject:(avviso di sicurezza) older_than:7d";
  let total = 0;
  for (let page = 0; page < 5; page++) {
    const listResp = await fetch(
      "https://gmail.googleapis.com/gmail/v1/users/me/messages?" + new URLSearchParams({ q, maxResults: "500" }),
  ...
  console.log("Cleanup Google security business: cestinate " + total);
}

// Cassa "X ricevute emesse" (le tue notifiche automatiche): restano visibili 7 giorni poi si
// cestinano da sole. E' solo una notifica riassuntiva, non la ricevuta stessa.
async function runCleanupCassaRicevute(env) {
  const account = "business";
  const tok = await getGmailAccessTokenFor(env, account);
  if (!tok || !tok.access_token) {
    console.log("Cleanup Cassa ricevute: auth fallita", tok);
    return;
  }
  const q = "subject:(Cassa) subject:(ricevute emesse) older_than:7d";
  let total = 0;
  for (let page = 0; page < 5; page++) {
    const listResp = await fetch(
      "https://gmail.googleapis.com/gmail/v1/users/me/messages?" + new URLSearchParams({ q, maxResults: "500" }),
  ...
  console.log("Cleanup Cassa ricevute: cestinate " + total);
}

// Amenitiz "Codice di autenticazione a due fattori" (business): restano visibili 7 giorni
// (servono davvero quando arrivano) poi si cestinano da soli, essendo codici monouso.
async function runCleanupAmenitiz2FA(env) {
  const account = "business";
  const tok = await getGmailAccessTokenFor(env, account);
  if (!tok || !tok.access_token) {
    console.log("Cleanup Amenitiz 2FA: auth fallita", tok);
    return;
  }
  const q = "from:amenitiz.io subject:(Codice di autenticazione a due fattori) older_than:7d";
  let total = 0;
  for (let page = 0; page < 5; page++) {
    const listResp = await fetch(
      "https://gmail.googleapis.com/gmail/v1/users/me/messages?" + new URLSearchParams({ q, maxResults: "500" }),
...
async function searchOneAccount(env, account, q, maxResults) {
  ...
  return { results };
      ...
      if (action === "runCleanupGoogleSecurityBusiness") {
        await runCleanupGoogleSecurityBusiness(env);
        return new Response(JSON.stringify({ ok: true }), { headers: { ...CORS, "Content-Type": "application/json" } });
      }

      if (action === "runCleanupCassaRicevute") {
        await runCleanupCassaRicevute(env);
        return new Response(JSON.stringify({ ok: true }), { headers: { ...CORS, "Content-Type": "application/json" } });
      }
