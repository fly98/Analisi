...
A pochi passi dalla Stazione Tiburtina (metro Linea B + treni Alta Velocità e regionali). Camere con bagno privato, angolo cottura, aria condizionata. Le camere 1, 2, 3 e 4 hanno la vasca idromassaggio (se non parte, controlla l'interruttore vicino alla luce del bagno); la camera 5 (Economy) ha la doccia, senza vasca.
...
- Gialla — Appartamento Bilocale fino a 4 persone, 18 m²: due ambienti separati. Nella camera un letto matrimoniale da 160 cm; nella seconda stanza un divano letto (può diventare anche due letti singoli) e l'angolo cottura (2 fuochi, frigorifero, microonde). Bagno con vasca idromassaggio con doccia, bidet. Aria condizionata in entrambe le stanze, TV 43" con Netflix. 5° letto su richiesta (20€ a notte).
...
- Camere 1 e 2 — Matrimoniale Superior con Terrazza, 2 persone, 15 m²: UN letto matrimoniale da 160 cm (non due letti singoli). Terrazza privata con tavolo e sedie, bagno con vasca idromassaggio, angolo cottura, TV 50" con Netflix. Si può aggiungere un 3° letto singolo su richiesta (20€ a notte).
- Camere 3 e 4 — Matrimoniale con Terrazza, 2 persone, 12 m²: un letto matrimoniale (misura non indicata), terrazza privata, bagno con vasca idromassaggio, angolo cottura, TV 32" con Netflix.
...
PRENOTARE: nel modulo di prenotazione si indica il numero totale di persone che soggiornano (per esempio 2 per una coppia). Si prenota direttamente su www.interno1.it/prenota, senza commissioni e con la tariffa migliore rispetto agli altri canali. Il concierge non vede la disponibilità in tempo reale: per date e prezzi rimandare sempre al sito. Se l'ospite ha difficoltà con la prenotazione sul sito (non riesce ad andare avanti, dubbi su cosa inserire), invitarlo a scrivere su WhatsApp +39 392 299 9914 o a info@interno1.it indicando date e numero di persone: lo aiutiamo noi.
`;

function systemPrompt(slug, lang) {
  const kb = KNOWLEDGE[slug] || KNOWLEDGE.campaldino;
  return `Sei il concierge digitale di InternoUno, un affittacamere a Roma. Rispondi SEMPRE e SOLO nella lingua in cui è scritto il messaggio dell'ospite, per tutta la risposta compreso il saluto iniziale (a chi scrive in inglese non dire "Ciao"), senza mescolare lingue (se il messaggio è troppo breve per capirlo usa "${lang}").
NON aggiungere mai dettagli che non sono scritti nelle informazioni qui sotto (misure dei letti, orari, servizi, prezzi): se un dettaglio non c'è, non inventarlo e, se serve, rimanda a WhatsApp. Usa per le camere esattamente i nomi e le caratteristiche elencati. Sii breve, cordiale, concreto: 2-4 frasi, no premesse.
