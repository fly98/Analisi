...
async function emettiFattura(env, dati) {
  ...
  try {
    ...
    // niente e' stato trasmesso: tolgo la copia scritta prima dell'invio,
    // altrimenti il numero risulta impegnato e viene saltato per sempre
    if (env.FISCO_KV) await env.FISCO_KV.delete(`fisco:fatt:${numeroDoc}`);
    throw e;
  }
  ...
  async fetch(request, env) {
    ...
    try {
            ...
            if (env.FISCO_KV) {
              ...
              if (raw && raw[0] === '{') {
                try {
                  ...
                  if (c.nome && c.nome !== 'Fornitore') cache = c;
                } catch {}
              }
            }
      ...
      if (url.pathname === '/fatture') {
        ...
        let cur = dal;
        while (cur <= al) {
          const fine = [cur.slice(0, 7), '28'].join('-');
          let ultimo = addGiorni(fine, 4);
          ultimo = ultimo.slice(0, 8) + '01';
          ultimo = addGiorni(ultimo, -1);
          const a = ultimo > al ? al : ultimo;
          blocchi.push([cur, a]);
          cur = addGiorni(a, 1);
        }
        ...
        for (const g of risposte)
          for (const f of g) {
            ...
            fatture.push({
              ...
              numero: f.numeroFattura,
              data: f.dataFattura,
              cliente: f.denominazioneCliente,
              piva: f.pivaCliente,
              imponibile: Math.round(imp * 100) / 100,
              imposta: Math.round(iva * 100) / 100,
              totale: Math.round((imp + iva) * 100) / 100,
              stato: f.stato,
              consegna: f.testoFattureConsegnate || '',
